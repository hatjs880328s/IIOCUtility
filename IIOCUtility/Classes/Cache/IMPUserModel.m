...
#define KLastLoginUser                  @"KLastLoginUser"
...
+ (IMPUserModel *)IIPRIVATE_lastLoginUserInfo {
    NSDictionary *dic = [[NSUserDefaults standardUserDefaults] objectForKey:KLastLoginUser];
    return [IMPUserModel mj_objectWithKeyValues:dic];
}

+ (void)setLastLoginUserInfo:(IMPUserModel *)user {
    if (user) {
        [[NSUserDefaults standardUserDefaults] setObject:[user mj_keyValues] forKey:KLastLoginUser];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }else{
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:KLastLoginUser];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}

+ (NSString *)IIPRIVATE_lastLoginUserAccountName {
    return [[NSUserDefaults standardUserDefaults] objectForKey:KLastLoginUserAccountName];
}

+ (void)IIPRIVATE_setLastLoginUserAccountName:(NSString *)accountName {
    if (accountName.length > 0) {
        [[NSUserDefaults standardUserDefaults] setObject:accountName forKey:KLastLoginUserAccountName];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
    else {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:KLastLoginUserAccountName];
        [[NSUserDefaults standardUserDefaults] synchronize];
    }
}
...
+ (void)IIPRIVATE_setActiveInstance:(IMPUserModel *)user {
    ...
    if(user.id != sharedInstance.id || oldEnterpriseModel.id != sharedInstance.enterprise.id) {
        [IMPCache reset];
        [[IIDataBaseManage instance] closeDataBase];
    }
    sharedInstance = user;
    if (user) {
